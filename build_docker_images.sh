...
ARM_IMAGE=""
AMD_IMAGE=""
IMAGE_PREFIX="registry.cn-hangzhou.aliyuncs.com/pubg/archery/archery"
...
示例:
  ./build_docker_images.sh -m all -v v1.14.1 \\
    --arm-image registry.cn-hangzhou.aliyuncs.com/pubg/archery/archery:v1.14.1-arm64 \\
...
build_arm() {
  run_cmd docker build --platform linux/arm64 -f src/docker/Dockerfile-base -t "$ARM_BASE_IMAGE" .
...
build_amd() {
  if [[ "$SKIP_AMD_BASE" == "false" ]]; then
    run_cmd docker build --platform linux/amd64 -f src/docker/Dockerfile-base -t "$AMD_BASE_IMAGE" .
  fi
